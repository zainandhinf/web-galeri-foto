...
@section('content')
    @if (session()->has('success'))
        <div class="alert alert-success mt-4 me-4 position-absolute end-0 z-1" role="alert">
            {{ session('success') }}
        </div>
    @elseif (session()->has('error'))
        <div class="alert alert-danger mt-4 me-4 position-absolute end-0 z-1" role="alert">
            {{ session('error') }}
        </div>
    @endif
                            ...
                            <div class="content-post"
                                ...
                                <div class="content-coment-post"
                                    ...
                                    @foreach ($photo_post->comments as $comment)
                                        ...
                                        @endphp
                                        @php
                                            $albumname = DB::table('albums')
                                                ->select('*')
                                                ->where('id', '=', $photo_post->album_id)
                                                ->get();
                                            // dd($photo_posts);
                                        @endphp
                                        <div class=""
                                                        ...
                                                        @if ($userComment[0]->id == auth()->user()->id)
                                                            <form action="/deletecomment" method="POST" class="">
                                                                ...
                                                                <input type="hidden" name="status"
                                                                    value="page_search_album">
                                                                <input type="hidden" name="slug"
                                                                    value="{{ $albumname[0]->slug }}">
                                                                <button type="submit"
                                ...
                                <div class="footer-content z-1"
                                        ...
                                        <div class="popup-left-side"
                                            ...
                                            @if ($photo_post->suka == 'BL')
                                                <form action="/addlike" method="POST">
                                                    ...
                                                    <input type="hidden" name="status" value="page_search_album">
                                    ...
                                    <div class="popup-comment-box"
                                        ...
                                        <form action="/addcomment" method="POST" style="width: 100%; display: flex">
                                            ...
                                            <input type="hidden" name="status" value="page_search_album">
                                            <input type="hidden" name="slug" value="{{ $albumname[0]->slug }}">
                                            <div class="popup-btn">
                                                <button
